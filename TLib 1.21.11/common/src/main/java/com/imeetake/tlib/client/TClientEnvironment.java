...
public class TClientEnvironment {
    ...
    public static float getRainStrength(float tickDelta) {
        ClientLevel level = getWorld();
        return level != null ? level.getRainLevel(tickDelta) : 0.0f;
    }
    ...
    public static boolean isColdBiome(Holder<Biome> entry, BlockPos pos) {
        ...
        try {
            return b.getPrecipitationAt(pos, seaLevel) == Biome.Precipitation.SNOW
    ...
    public static boolean isAridBiome(Holder<Biome> entry, BlockPos pos) {
        ...
        int seaLevel = level != null ? level.getSeaLevel() : 63;
        try {
            return b.getPrecipitationAt(pos, seaLevel) == Biome.Precipitation.NONE;
        } catch (Throwable t) {
            return false;
        }
    }
